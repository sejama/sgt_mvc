...
class TablaManager {
    ...
    {
        return $this->cache->get('posiciones_grupo_' . $grupo->getId(), function (ItemInterface $item) use ($grupo) {
    ...
    public function clearCache(int $grupoId): void
    {
        $this->cache->delete('posiciones_grupo_' . $grupoId);
    }
}