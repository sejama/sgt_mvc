...
class RegistrarController extends AbstractController
{
    ...
    #[Route('/cambiar_password', name: 'app_cambiar_password', methods: ['GET', 'POST'])]
    ...
    public function editar(Request $request, RegistrarManager $usuarioManager, LoggerInterface $logger, $id): Response
    {
        ...
        if ($request->isMethod('POST')) {
            try{
                ...
                $usuarioManager->editarUsuario($usuario, $nombre, $apellido, $email, $roles);
    ...
    public function eliminar(RegistrarManager $usuarioManager, LoggerInterface $logger, $id): Response
    {
        try{
            $usuario = $usuarioManager->buscarUsuario((int)$id);
